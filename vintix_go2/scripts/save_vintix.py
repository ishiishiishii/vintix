...
def generate_output_filename(vintix_path: str, robot_type: str, num_envs: int, max_episodes: int, output_dir: str = None, base_mass: float = None):
    ...
    robot_type = _canon_robot_type(robot_type)
...
def _canon_robot_type(robot_type: str) -> str:
    t = robot_type.strip().lower()
...
def _create_env(robot_type: str, num_envs: int, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer: bool = False):
    """環境を作成するヘルパー関数"""
    rt = _canon_robot_type(robot_type)
    env_classes = {
        "go2": Go2Env,
    ...
    if rt not in env_classes:
        raise ValueError(f"Unknown robot type: {robot_type}")
    
    return env_classes[rt](
        num_envs=num_envs,
        env_cfg=env_cfg,
        obs_cfg=obs_cfg,
...
def _get_task_name_for_robot(robot_type: str, model_metadata: dict):
    ...
    legacy_a1 = "unitreea1_walking_ad"

    task_name = task_name_map.get(rt)
    if task_name is None:
        raise ValueError(f"Unknown robot_type: {robot_type}")

    if task_name in model_metadata:
        return task_name, False
    if task_name == "a1_walking_ad" and legacy_a1 in model_metadata:
        return legacy_a1, False
...
def main():
    ...
    parser.add_argument("-r", "--robot_type", type=str, choices=["go2", "minicheetah", "laikago", "go1", "a1", "unitreea1", "anymalc"], 
    ...
    def get_exp_name_for_robot(robot_type):
        if robot_type == "go2":
            ...
            return "go1-walking"
        elif robot_type in ("a1", "unitreea1"):
            return "a1-walking"
    ...
    for robot_idx, robot_type in enumerate(robot_types, 1):
        ...
        if cfgs_path.exists():
            ...
            elif _canon_robot_type(robot_type) == "a1":
