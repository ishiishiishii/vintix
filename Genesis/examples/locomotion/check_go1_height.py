"""
UnitreeGo1の初期関節角度で立たせたときの実際の高さを測定するスクリプト
train.pyと同じ設定を使用
...
# train.pyと同じデフォルト関節角度（get_go1_cfgs()から）
...
# train.pyと同じジョイント名の順序（get_go1_cfgs()から）
joint_names = [
    "FR_hip_joint",
    "FR_thigh_joint",
    "FR_calf_joint",
...
base_init_pos = [0.0, 0.0, 0.4]  # 初期値（後で調整）
base_init_quat = [1.0, 0.0, 0.0, 0.0]  # train.pyと同じ

# train.pyと同じ目標高さ（get_go1_cfgs()から）
base_height_target = 0.32  # 目標高さ0.32m（32cm）
